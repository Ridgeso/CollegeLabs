...
MapPoint::MapPoint(const MapPoint& point)
    : m_latitude(point.m_latitude), m_longitude(point.m_longitude)
{
    N_MAP_POINT(true);
}

MapPoint::MapPoint(double latitude, double longitude)
...
MapPoint::~MapPoint()
{
    // jak o to nie dbamy to nie dbamy XD
    // s_allPoints--;
}
...
int MapPoint::N_MAP_POINT(bool inc)
{
    static int s_allPoints;
    return inc == true ? s_allPoints++ : s_allPoints;
}
