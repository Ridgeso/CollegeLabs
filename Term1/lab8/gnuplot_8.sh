...
set term png size 1024, 768 enhanced font "Helvetica, 36"
...
do for [i=0:2]{
    set output "f1_n".i.".png" #.eps
    plot [-5:5][] f1(x) w l ls 1 t "f_1(x) = ^1/_{(1+x^2)}",\
      "f1.dat" i i u 1:2 w l ls 2 t "s(x), n = ".i.""
}
...
do for [i=0:2]{
    set output "f2_n".i.".png" #.eps
    plot [-5:5][] f2(x) w l ls 1 t "f_2(x) = cos(2x)",\
      "f2.dat" i i u 1:2 w l ls 2 t "s(x), n = ".i.""
}
